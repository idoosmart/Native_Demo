...
/**
 * @brief 模拟器接收到APP指令后，将APP二进制数据传入这个接口，输出模拟器应该回复的二进制数据和对应的json内容
 * @param data_get APP下发的字节数据
 * @param length APP下发字节数据的长度
 * @param type 待定，SPP标志
 * @return 0成功 非0失败
 */
extern int SimulatorDataExec(uint8_t const *data,uint16_t length,uint8_t type);
