...
class FunctionPageVC: UIViewController {
    ...
    deinit {
        ...
        sdk.bridge.markDisconnectedDevice(macAddress: deviceModel?.macAddress, uuid: self.deviceModel?.uuid) {_ in }
    }
}
...
extension FunctionPageVC {
    ...
    private func onDeviceStateChanged() {
        ...
        if deviceState.state == .connected, deviceState.macAddress != nil, deviceState.macAddress!.count > 0 {
            ...
            let otaType = deviceModel.isTlwOta ?? false
                ? IDOOtaType.telink
                : deviceModel.isOta ?? false
                    ? IDOOtaType.nordic
                    : IDOOtaType.none;
            sdk.bridge.markConnectedDevice(uniqueId: uniqueId, otaType: otaType, isBinded: isBinded, deviceName: deviceModel.name) { [weak self] rs in
    ...
    private func _disconnect() {
        ...
        sdk.bridge.markDisconnectedDevice(macAddress: self.deviceModel?.macAddress, uuid: self.deviceModel?.uuid) { [weak self] rs in
